const getReply = async (message, params, userFromDB) => {
  ...
  if (userFromDB.data) {
    if (params.prevFlow === 'introduction' && params.currentEntity !== 'fromIntroPreTutorFlow') {
      ...
      await API.updateFlow(message.senderId, { tutor_flow_status: 'requested' })
  ...
  if (userLevel === 1) {
    ...
    case 'daysGroupForCalls':
      ...
      if (params.rawUserInput === 'TFB_NO_ADD') {
        ...
        if (userFromRedis) {
          ...
          await BotCache.saveUserDataCache(message.senderId, message.conversationId, message.userHash, params.currentFlow, params.prevPos, tutorCallDaytime)
        }
      }
      reminderToContinueOn = true
      break
    ...
    case 'userHasNoMoney':
      ...
      await BotCache.saveUserDataCache(message.senderId, message.conversationId, message.userHash, params.currentFlow, params.currentEntity, params.rawUserInput)
      break
  ...
  if (flowControlUpdate) {
    API.updateFlow(message.senderId, flowControlUpdate)
  }
  ...
  if (reminderToContinueOn) {
    ...
    controllerSmash.CronReminder(params.currentEntity, futureRepliesToSend, futureMsgTime, futureMsgFlowUpdate, message.senderId, userFromDB)
  }
